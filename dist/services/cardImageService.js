...
exports.buildCardListImage = buildCardListImage;
const canvas_1 = require("@napi-rs/canvas");
const registerFonts_1 = require("../utils/registerFonts");
(0, registerFonts_1.registerCanvasFonts)();
...
function sanitizeForCanvas(text) {
    ...
    result = result.replace(/[^\x20-\x7E\u00A0-\u024F\u1E00-\u1EFF\u2010-\u2027\u2030-\u205E\u2600-\u27BF\u{1F1E6}-\u{1F1FF}\u{1F300}-\u{1FAFF}\u200D\uFE0F]/gu, "");
    return result.trim();
}
...
async function tryLoadImage(url) {
    ...
    try {
        ...
        return await (0, canvas_1.loadImage)(Buffer.from(arrayBuffer));
    }
    catch {
        return null;
    }
}
function drawAvatarCircle(ctx, x, y, rank, avatarImage) {
    if (avatarImage) {
        ctx.save();
        ctx.beginPath();
...
function drawEntry(ctx, entry, columnX, y, avatarImage) {
 ...
 */
async function buildCardListImage(options, entries) {
    ...
    const preparedEntries = entries.map((entry) => {
        ...
        const detailLines = entry.detailLines.flatMap((line) => wrapTextToLines(measureCtx, sanitizeForCanvas(line), maxTextWidth, 2));
