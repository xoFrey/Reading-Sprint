 ...
 */
function isConsecutiveDay(lastDate, now) {
    if (!lastDate)
        return false;
    const last = new Date(lastDate);
    const diffInDays = Math.floor((stripTime(now).getTime() - stripTime(last).getTime()) / (1000 * 60 * 60 * 24));
    return diffInDays === 1;
}
 ...
 */
function updateStreak(user, pagesRead, now = new Date()) {
    if (!(0, xpService_1.isStreakEligible)(pagesRead))
        return;
