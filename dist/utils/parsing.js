...
exports.parseTimeRelativeToNow = parseTimeRelativeToNow;
...
function parseTimeRelativeToNow(timeStr) {
    const timeMatch = timeStr.trim().match(/^(\d{1,2}):(\d{2})$/);
    if (!timeMatch)
        return null;
    const [, hour, minute] = timeMatch;
    const now = new Date();
    const result = new Date(now.getFullYear(), now.getMonth(), now.getDate(), Number(hour), Number(minute));
