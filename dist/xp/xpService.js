...
exports.isStreakEligible = isStreakEligible;
exports.calculateSprintXP = calculateSprintXP;
exports.applyXPGain = applyXPGain;
const levelCurve_1 = require("./levelCurve");
...
function isStreakEligible(pagesRead) {
    return pagesRead >= xpConfig_1.MIN_PAGES_FOR_STREAK;
}
function calculateSprintXP(config, input) {
    let xp = 0;
    ...
    const streakEligible = input.pagesRead >= xpConfig_1.MIN_PAGES_FOR_STREAK;
...
function applyXPGain(user, amount) {
    ...
    if (!Number.isFinite(user.xp))
        user.xp = 0;
    const oldLevel = (0, levelCurve_1.calculateLevelProgress)(user.xp).level;
    user.xp += amount;
    const newLevel = (0, levelCurve_1.calculateLevelProgress)(user.xp).level;
    user.level = newLevel;
    return {
